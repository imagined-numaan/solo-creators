...
export default function initEnv() {
  const envConfig: Env = {
    ...
    LOCAL_MONGO_URI: process.env.MONGO_URI || "",
    SOLO_URI: process.env.SOLO_URI || "",
  ...
  if (!envConfig.LOCAL_MONGO_URI) {
    throw new Error("LOCAL_MONGO_URI is not set");
  }

  if (!envConfig.SOLO_URI) {
    throw new Error("SOLO_URI is not set");
  }
